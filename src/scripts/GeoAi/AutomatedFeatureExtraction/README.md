...
3. **Classify Similarity** runs an independent similarity search for every populated value in an example-point class field, then merges the results. The output includes `AUTO_CLASS`, containing the value from the source field.
...
A cell can appear once for each matching class. This preserves the model evidence when visual concepts overlap instead of forcing a winner before review.
...
**Output Features** is the main result: extracted polygons for Feature Extraction, matching embedding cells for Embedding Similarity, or matching cells labeled with `AUTO_CLASS` for Classify Similarity. The derived Embeddings and Similar Features outputs can be retained for inspection or reruns. Store output feature classes in a file or enterprise geodatabase because embeddings contain BLOB fields.
