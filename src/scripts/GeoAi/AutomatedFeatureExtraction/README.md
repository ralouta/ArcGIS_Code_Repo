...
1. **Feature Extraction** uses Living Atlas SAM3 to create polygon evidence for a controlled topographic profile. Persistent candidate profiles include Buildings, Bridges, Roads, Water Bodies, Rail Corridors, Impervious Surfaces, Parking Areas, Solar Arrays, Sports Surfaces, Swimming Pools, Trees, Forest Cover, and Other Structures. Roads are published as polyline centerline candidates after mask repair, conservative simplification, observed-component dissolve, polygon-native centerline extraction, high-confidence direction-compatible gap connectors, $5 m point-removal line simplification, and $8 m PAEK smoothing with fixed endpoints. Road endpoint direction is measured across a 15 m tangent window, avoiding a decision based on a single noisy centerline vertex. Gaps within 50 m may align within 45 degrees; occlusions up to 100 m require both road tangents to point toward the gap within 20 degrees; and gaps up to 200 m require near-collinear alignment within 12 degrees. A connector must be the clear best match for both endpoints and must not cross another road or connector. Connected chains are merged into continuous line features and fragments under 5 m are removed. If every fragment is removed, the tool fails without publishing an empty road layer. `ROAD_WIDTH_M` is the component mask area divided by total centerline length. Longer or directionally ambiguous occlusions remain disconnected and require directional, width, and imagery review. Agricultural Fields use repair, area screening, shrink-clean-expand processing, boundary simplification, and smoothing to derive parcel-scale candidates. If either workflow cannot complete QA, it fails without publishing an uncleaned SAM3-mask fallback. Agricultural Fields, Park-Like Green Space, Construction Areas, Material Stockpiles, Bare Ground, Flooded Areas, Debris, Vehicles, and Utility Poles are observation or change-monitoring profiles, not stable base-map features. Park-Like Green Space represents visible vegetation and amenities; it cannot establish a legal park or land-use designation. `Custom` remains evidence-only until its feature code, geometry, and QA rules have been governed.
...
3. **Feature Classification** classifies target features using an independent similarity search for every populated value in an example-point class field. Supply **Input Features to Classify** to classify existing polygons or polylines, or leave it blank to extract target features with SAM3 before classification. The output is the target feature layer with `AUTO_CLASS` and `CLASS_COV_PCT`, the strongest matching class evidence by area for polygons and by length for road centerlines.
...
`AutomatedFeatureExtraction.pyt` remains the ArcGIS Pro toolbox entry point and workflow coordinator. Shared ArcPy parameter construction lives in `parameter_helpers.py`; field, workspace, unit, and coverage validation lives in `validation_helpers.py`; feature-specific geometry cleanup and regularization lives in `feature_geometry.py`; SAM3 detection and mask filtering lives in `feature_extraction.py`; per-class query selection lives in `query_selection.py`; and damage-evidence output construction lives in `damage_classification.py`. New reusable toolbox behavior should be added to a focused Python module rather than expanding the entry point.

## Outputs

**Output Features** is the main result: candidate polygons for most Feature Extraction profiles, road centerline polylines for Roads, matching embedding cells for Embedding Similarity, or classified target features for Feature Classification. It is an auditable candidate layer, never an automatically accepted authoritative topographic update. Every output records `AFE_RUN_ID`, `FEATURE_CODE`, `FEATURE_TYPE`, `GEOM_ROLE`, `QC_STATUS`, `QC_REASON`, `TOOL_VERSION`, `PROFILE_VER`, `SOURCE_IMAGE`, `MODEL_ITEM_ID`, `MODEL_FILE`, `RUN_UTC`, and `AREA_SQM`. Road centerlines also record `LENGTH_M`, `ROAD_LENGTH_M`, `ROAD_WIDTH_M`, `WIDTH_METHOD`, and `AFE_COMPONENT_ID`.
